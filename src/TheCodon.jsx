...
:root { --bg:#050508; --cyan:#00FFD1; --violet:#7B2FBE; --warn:#FF6B35; --gold:#FFD700; }
html { scroll-behavior: smooth; }
body { background:var(--bg); color:#e8e8f0; font-family:'DM Sans',sans-serif; overflow-x:hidden; }
...
@keyframes glow-pulse { 0%,100%{box-shadow:0 0 20px rgba(0,255,209,.3)} 50%{box-shadow:0 0 50px rgba(0,255,209,.7),0 0 100px rgba(0,255,209,.2)} }
...
.btn-primary {
  ...
  box-shadow:0 0 40px rgba(0,255,209,.4); animation:glow-pulse 3s infinite;
  display:inline-flex; align-items:center; justify-content:center;
  text-decoration:none; line-height:1; white-space:nowrap;
  -webkit-appearance:none; appearance:none;
  user-select:none; -webkit-tap-highlight-color:transparent;
}
.btn-primary:hover { transform:translateY(-2px); box-shadow:0 8px 60px rgba(0,255,209,.7); }
.btn-primary:active { transform:translateY(0px) scale(0.98); }
.btn-ghost {
  background:transparent !important; color:var(--cyan) !important;
  border:1px solid rgba(0,255,209,.4); border-radius:6px;
  padding:16px 36px; font-size:15px; font-family:'Syne',sans-serif;
...
.btn-ghost:hover { border-color:var(--cyan) !important; background:rgba(0,255,209,.08) !important; transform:translateY(-2px); }
...
function CodonLogo({ size = 32, showText = true, textSize = 18 }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, userSelect: "none" }}>
      {/* Icon: three stacked base-pair dots forming a codon triplet */}
      <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        {/* Outer glow ring */}
        <circle cx="20" cy="20" r="19" stroke="rgba(0,255,209,0.15)" strokeWidth="1"/>
        {/* Left strand dots */}
        <circle cx="11" cy="12" r="3.2" fill="#00FFD1" opacity="0.9"/>
        <circle cx="11" cy="20" r="3.2" fill="#00FFD1" opacity="0.6"/>
        <circle cx="11" cy="28" r="3.2" fill="#00FFD1" opacity="0.35"/>
        {/* Right strand dots */}
        <circle cx="29" cy="12" r="3.2" fill="#7B2FBE" opacity="0.9"/>
        <circle cx="29" cy="20" r="3.2" fill="#7B2FBE" opacity="0.6"/>
        <circle cx="29" cy="28" r="3.2" fill="#7B2FBE" opacity="0.35"/>
        {/* Connecting rungs */}
        <line x1="14.2" y1="12" x2="25.8" y2="12" stroke="rgba(0,255,209,0.5)" strokeWidth="1.2"/>
        <line x1="14.2" y1="20" x2="25.8" y2="20" stroke="rgba(0,255,209,0.3)" strokeWidth="1.2"/>
        <line x1="14.2" y1="28" x2="25.8" y2="28" stroke="rgba(0,255,209,0.15)" strokeWidth="1.2"/>
        {/* Left vertical strand */}
        <line x1="11" y1="15.2" x2="11" y2="16.8" stroke="rgba(0,255,209,0.4)" strokeWidth="1.5"/>
        <line x1="11" y1="23.2" x2="11" y2="24.8" stroke="rgba(0,255,209,0.25)" strokeWidth="1.5"/>
        {/* Right vertical strand */}
        <line x1="29" y1="15.2" x2="29" y2="16.8" stroke="rgba(123,47,190,0.4)" strokeWidth="1.5"/>
        <line x1="29" y1="23.2" x2="29" y2="24.8" stroke="rgba(123,47,190,0.25)" strokeWidth="1.5"/>
      </svg>
      {showText && (
        <div style={{ display: "flex", alignItems: "baseline", gap: 1 }}>
          ...
          <span style={{
            fontFamily: "'Syne',sans-serif", fontWeight: 800,
            fontSize: textSize + 2, color: "#00FFD1",
...
function HelixCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      fog.addColorStop(0, "rgba(0,255,209,0.05)");
      ...
      for (let i = 0; i <= nodes; i++) {
        ...
        ctx.strokeStyle = `rgba(0,255,209,${0.12 + 0.12 * ((p1.z + 1) / 2)})`; ctx.lineWidth = 0.8; ctx.stroke();
        [p1, p2].forEach((p, si) => {
          const r = 3.5 + 2 * ((p.z + 1) / 2);
          const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, r * 2.5);
          const a2 = 0.8 * ((p.z + 1) / 2 + 0.3);
          if (si === 0) { g.addColorStop(0, `rgba(0,255,209,${a2})`); g.addColorStop(1, "rgba(0,255,209,0)"); }
      ...
      [s1, s2].forEach((strand, si) => {
        ...
        ctx.strokeStyle = si === 0 ? "rgba(0,255,209,0.85)" : "rgba(123,47,190,0.85)";
        ctx.lineWidth = 1.8; ctx.shadowColor = si === 0 ? "#00FFD1" : "#7B2FBE";
      ...
      for (let p = 0; p < 28; p++) {
        ...
        ctx.fillStyle = `rgba(0,255,209,${0.15 + 0.25 * Math.sin(t * 1.2 + p)})`; ctx.fill();
      }
      t += 0.012;
  ...
  return (
    <nav style={{
      ...
      <div className="desktop-nav" style={{ display: "flex", gap: 32, alignItems: "center" }}>
        ...
        <button onClick={() => scrollTo("contact")} className="btn-primary" style={{ padding: "9px 22px", fontSize: 13, animation: "none", boxShadow: "0 0 20px rgba(0,255,209,0.3)", borderRadius: 5, color: "#050508" }}>Claim 50% Off →</button>
      </div>
      <button className="hamburger" onClick={() => setOpen(!open)} style={{ display: "none", flexDirection: "column", gap: 5, background: "none", border: "none", cursor: "pointer", padding: 8 }}>
        {[0, 1, 2].map(i => (
          <span key={i} style={{ display: "block", width: 22, height: 2, background: "#00FFD1", borderRadius: 1, transition: "all 0.3s", transform: open ? (i === 0 ? "rotate(45deg) translate(5px,5px)" : i === 1 ? "scaleX(0)" : "rotate(-45deg) translate(5px,-5px)") : "none" }} />
        ))}
      </button>
      {open && (
...
function Hero() {
  ...
  return (
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "140px 5% 80px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: "10%", left: "5%", width: 280, height: 280, background: "radial-gradient(circle, rgba(0,255,209,0.07) 0%, transparent 70%)", pointerEvents: "none" }} />
      <div className="hero-grid" style={{ display: "flex", alignItems: "center", width: "100%", gap: "4%" }}>
        ...
        <div style={{ flex: "1 1 400px", minWidth: 280 }}>
          ...
          {["YOUR BRAND,", "DECODED &", "EXPRESSED."].map((line, i) => (
            <h1 key={i} style={{ fontSize: "clamp(36px,5vw,72px)", fontWeight: 800, lineHeight: 1.05, letterSpacing: "-0.02em", color: i === 2 ? "#00FFD1" : "#e8e8f0", opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(50px)", transition: `all 0.9s cubic-bezier(0.16,1,0.3,1) ${0.2 + i * 0.15}s` }}>{line}</h1>
          ))}
          ...
          <div style={{ marginTop: 28, display: "inline-flex", alignItems: "flex-start", gap: 12, background: "rgba(0,255,209,0.04)", border: "1px solid rgba(0,255,209,0.12)", borderRadius: 12, padding: "16px 20px", maxWidth: 500, opacity: vis ? 1 : 0, transition: "opacity 0.9s ease 0.75s" }}>
            <span style={{ fontSize: 18, flexShrink: 0, marginTop: 1 }}>💡</span>
            <p style={{ fontSize: 14, color: "rgba(232,232,240,0.65)", lineHeight: 1.75 }}>
              We're brand new — <strong style={{ color: "#00FFD1" }}>0 projects delivered.</strong> So our first 10 clients get <strong style={{ color: "#FF6B35" }}>50% off everything</strong> — you help us build a portfolio, we deliver premium work at half price.
            </p>
          </div>
          <div style={{ display: "flex", gap: 14, marginTop: 36, flexWrap: "wrap", opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(30px)", transition: "all 0.9s cubic-bezier(0.16,1,0.3,1) 0.85s" }}>
            <button onClick={() => document.getElementById("pricing")?.scrollIntoView({ behavior: "smooth" })} className="btn-primary" style={{ flex: "1 1 auto", maxWidth: 320, fontSize: 14 }}>See Prices & Claim Offer</button>
...
function Marquee() {
  ...
  return (
    <div style={{ borderTop: "1px solid rgba(0,255,209,0.1)", borderBottom: "1px solid rgba(0,255,209,0.1)", padding: "14px 0", overflow: "hidden", background: "rgba(0,255,209,0.015)" }}>
      <div style={{ display: "flex", whiteSpace: "nowrap", animation: "marquee 28s linear infinite" }}>
        {[0, 1].map(i => <span key={i} style={{ fontFamily: "'Syne',sans-serif", fontSize: 12, letterSpacing: "0.25em", color: "#00FFD1", paddingRight: 60, fontWeight: 700 }}>{t}</span>)}
      </div>
    </div>
  );
}
    ...
    <section ref={ref} style={{ padding: "100px 5%", background: "rgba(255,107,53,0.03)", borderTop: "1px solid rgba(255,107,53,0.08)", borderBottom: "1px solid rgba(255,107,53,0.08)" }}>
      <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
        ...
        <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(28px,4vw,52px)", fontWeight: 800, color: "#e8e8f0", lineHeight: 1.2, marginBottom: 28, opacity: inView ? 1 : 0, transform: inView ? "none" : "translateY(30px)", transition: "all 0.8s ease 0.1s" }}>
          We have <span style={{ color: "#00FFD1" }}>0 clients.</span><br />That's exactly why you should hire us.
        </h2>
    ...
    <section id="services" ref={ref} style={{ padding: "100px 5%" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 56, opacity: inView ? 1 : 0, transition: "opacity 0.7s" }}>
          <p style={{ color: "#00FFD1", fontSize: 12, letterSpacing: "0.3em", marginBottom: 14, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>◈ WHAT WE BUILD</p>
...
function ServiceCard({ s, i, inView }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ background: hov ? "rgba(0,255,209,0.04)" : "rgba(255,255,255,0.025)", border: `1px solid ${hov ? "rgba(0,255,209,0.45)" : "rgba(255,255,255,0.06)"}`, borderRadius: 12, padding: "36px 30px", transition: "all 0.35s cubic-bezier(0.16,1,0.3,1)", transform: hov ? "translateY(-8px)" : inView ? "none" : "translateY(40px)", opacity: inView ? 1 : 0, transitionDelay: `${0.08 * i}s`, boxShadow: hov ? "0 20px 60px rgba(0,255,209,0.08)" : "none" }}>
      <div style={{ fontSize: 26, marginBottom: 12 }}>{s.icon}</div>
      <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 44, fontWeight: 800, color: "rgba(0,255,209,0.1)", lineHeight: 1, marginBottom: 14 }}>{s.n}</div>
      <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: 18, fontWeight: 700, color: "#e8e8f0", marginBottom: 10 }}>{s.name}</h3>
      <p style={{ fontSize: 14, color: "rgba(232,232,240,0.5)", lineHeight: 1.75, marginBottom: 22 }}>{s.desc}</p>
      <ul style={{ listStyle: "none" }}>
        {s.deliverables.map((d, j) => (
          <li key={j} style={{ fontSize: 13, color: "rgba(232,232,240,0.6)", padding: "7px 0", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ color: "#00FFD1", fontSize: 9 }}>▸</span>{d}
          </li>
        ))}
      </ul>
    </div>
    ...
    <section id="pricing" ref={ref} style={{ padding: "100px 5%", background: "rgba(0,0,0,0.3)" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 28, opacity: inView ? 1 : 0, transition: "opacity 0.7s" }}>
          <p style={{ color: "#00FFD1", fontSize: 12, letterSpacing: "0.3em", marginBottom: 14, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>◈ TRANSPARENT PRICING</p>
          <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "clamp(32px,5vw,56px)", fontWeight: 800, color: "#e8e8f0", letterSpacing: "-0.02em", marginBottom: 18 }}>
            NO HIDDEN FEES.<br /><span style={{ color: "#00FFD1" }}>EVERYTHING UPFRONT.</span>
          </h2>
        ...
        <div className="pricing-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(290px,1fr))", gap: 20 }}>
...
function PricingCard({ plan, i, inView, offerActive }) {
  ...
  const bc = plan.gold ? "#FFD700" : plan.popular ? "#00FFD1" : "rgba(255,255,255,0.07)";
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      background: plan.popular ? "rgba(0,255,209,0.04)" : plan.gold ? "rgba(255,215,0,0.03)" : "rgba(255,255,255,0.025)",
      border: `1px solid ${hov ? (plan.gold ? "#FFD700" : "#00FFD1") : bc}`,
      ...
      boxShadow: hov ? `0 24px 60px ${plan.gold ? "rgba(255,215,0,0.12)" : "rgba(0,255,209,0.1)"}` : plan.popular ? "0 0 40px rgba(0,255,209,0.08)" : "none",
      display: "flex", flexDirection: "column",
    }}>
      {plan.tag && (
        <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", background: plan.gold ? "#FFD700" : plan.popular ? "#00FFD1" : "rgba(255,107,53,0.9)", color: (plan.gold || plan.popular) ? "#050508" : "#fff", padding: "4px 14px", borderRadius: 100, fontSize: 10, fontFamily: "'Syne',sans-serif", fontWeight: 800, letterSpacing: "0.12em", whiteSpace: "nowrap" }}>{plan.tag}</div>
      )}
      <div style={{ fontSize: 26, marginBottom: 14 }}>{plan.icon}</div>
      <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: 19, fontWeight: 800, color: "#e8e8f0", marginBottom: 20 }}>{plan.name}</h3>
      <div style={{ marginBottom: 26 }}>
        {offerActive ? (
          <>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 46, fontWeight: 800, color: plan.gold ? "#FFD700" : "#00FFD1", lineHeight: 1 }}>${discounted}</span>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 46, fontWeight: 800, color: "#00FFD1", lineHeight: 1 }}>${plan.full}</span>
            <span style={{ fontSize: 13, color: "rgba(232,232,240,0.4)" }}>/ project</span>
          </div>
        )}
      </div>
      <ul style={{ listStyle: "none", flex: 1, marginBottom: 26 }}>
        {plan.features.map((f, j) => (
          <li key={j} style={{ fontSize: 13, color: "rgba(232,232,240,0.65)", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ color: plan.gold ? "#FFD700" : "#00FFD1", fontSize: 11, flexShrink: 0 }}>✓</span>{f}
          </li>
        ))}
      </ul>
      <a href="#contact" style={{ display: "block", textAlign: "center", background: plan.gold ? "#FFD700" : plan.popular ? "#00FFD1" : "transparent", color: (plan.gold || plan.popular) ? "#050508" : "#00FFD1", border: (!plan.gold && !plan.popular) ? "1px solid rgba(0,255,209,0.35)" : "none", borderRadius: 8, padding: "14px 16px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 13, letterSpacing: "0.08em", transition: "all 0.25s", cursor: "pointer", boxShadow: plan.popular ? "0 0 30px rgba(0,255,209,0.2)" : plan.gold ? "0 0 30px rgba(255,215,0,0.2)" : "none", userSelect: "none", WebkitTapHighlightColor: "transparent" }}
        onMouseEnter={e => { const t = e.currentTarget; if (!plan.popular && !plan.gold) { t.style.background = "rgba(0,255,209,0.08)"; t.style.borderColor = "#00FFD1"; } t.style.transform = "translateY(-2px)"; }}
        onMouseLeave={e => { const t = e.currentTarget; if (!plan.popular && !plan.gold) { t.style.background = "transparent"; t.style.borderColor = "rgba(0,255,209,0.35)"; } t.style.transform = "none"; }}
...
function Process() {
  ...
  return (
    <section id="process" ref={ref} style={{ padding: "100px 5%" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 56, opacity: inView ? 1 : 0, transition: "opacity 0.7s" }}>
          <p style={{ color: "#00FFD1", fontSize: 12, letterSpacing: "0.3em", marginBottom: 14, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>◈ HOW IT WORKS</p>
        ...
        <div className="process-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 2, borderTop: "1px solid rgba(0,255,209,0.1)" }}>
          {steps.map((s, i) => (
            <div key={i} style={{ padding: "44px 28px", borderRight: i < 3 ? "1px solid rgba(0,255,209,0.07)" : "none", opacity: inView ? 1 : 0, transform: inView ? "none" : "translateX(-24px)", transition: `all 0.7s cubic-bezier(0.16,1,0.3,1) ${0.15 + i * 0.15}s` }}>
              <div style={{ fontSize: 26, marginBottom: 14 }}>{s.icon}</div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 40, fontWeight: 800, color: "rgba(0,255,209,0.12)", lineHeight: 1, marginBottom: 14 }}>{s.n}</div>
              <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: 19, fontWeight: 700, color: "#e8e8f0", marginBottom: 10 }}>{s.title}</h3>
              <p style={{ fontSize: 14, color: "rgba(232,232,240,0.5)", lineHeight: 1.75 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section ref={ref} style={{ padding: "100px 5%", background: "rgba(0,0,0,0.2)" }}>
      <div style={{ maxWidth: 780, margin: "0 auto" }}>
        <div style={{ marginBottom: 56, opacity: inView ? 1 : 0, transition: "opacity 0.7s" }}>
          <p style={{ color: "#00FFD1", fontSize: 12, letterSpacing: "0.3em", marginBottom: 14, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>◈ FAQ</p>
        ...
        {faqs.map((f, i) => (
          <div key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", opacity: inView ? 1 : 0, transition: `opacity 0.6s ease ${0.06 * i}s` }}>
            <button onClick={() => setOpen(open === i ? null : i)} style={{ width: "100%", background: "none", border: "none", padding: "22px 0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, cursor: "pointer", textAlign: "left" }}>
              ...
              <span style={{ flexShrink: 0, width: 24, height: 24, borderRadius: "50%", border: "1px solid rgba(0,255,209,0.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#00FFD1", fontSize: 16, transition: "transform 0.3s", transform: open === i ? "rotate(45deg)" : "none" }}>+</span>
            </button>
...
function Contact() {
  ...
  const inp = (id) => ({
    width: "100%", background: "rgba(0,255,209,0.025)",
    border: `1px solid ${focused === id ? "#00FFD1" : "rgba(0,255,209,0.12)"}`,
    borderRadius: 6, padding: "13px 16px", color: "#e8e8f0",
    fontSize: 14, fontFamily: "'DM Sans',sans-serif",
    outline: "none", transition: "border-color 0.2s",
    boxShadow: focused === id ? "0 0 20px rgba(0,255,209,0.08)" : "none",
  });
  ...
  return (
    <section id="contact" ref={ref} style={{ padding: "100px 5%", background: "rgba(0,0,0,0.4)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ marginBottom: 56, opacity: inView ? 1 : 0, transition: "opacity 0.7s" }}>
          <p style={{ color: "#00FFD1", fontSize: 12, letterSpacing: "0.3em", marginBottom: 14, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>◈ GET STARTED</p>
        ...
        <div className="contact-grid" style={{ display: "flex", gap: "6%", flexWrap: "wrap", opacity: inView ? 1 : 0, transition: "opacity 0.7s 0.2s" }}>
          ...
          <div style={{ flex: "1 1 250px", marginBottom: 40 }}>
            ...
            <div style={{ borderLeft: "2px solid rgba(0,255,209,0.15)", paddingLeft: 18, marginBottom: 32 }}>
              {["We receive your message", "Reply within 24h with a clear proposal", "You approve scope & pay 50% deposit", "We start building immediately"].map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "flex-start" }}>
                  <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 11, color: "#00FFD1", marginTop: 3, flexShrink: 0 }}>0{i + 1}</span>
                  <span style={{ fontSize: 13, color: "rgba(232,232,240,0.6)", lineHeight: 1.6 }}>{s}</span>
                </div>
              ))}
            </div>
            <a href="mailto:hello@thecodon.co" style={{ color: "#00FFD1", fontSize: 14, fontFamily: "'Syne',sans-serif", fontWeight: 700, display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#00FFD1", boxShadow: "0 0 10px #00FFD1", animation: "pulse 2s infinite", display: "inline-block" }} />
              hello@thecodon.co
            </a>
          </div>
          {/* Form */}
          <div style={{ flex: "1 1 380px", background: "rgba(0,255,209,0.015)", border: "1px solid rgba(0,255,209,0.1)", borderRadius: 14, padding: "38px 34px" }}>
            <div style={{ display: "flex", gap: 6, marginBottom: 26 }}>
              {["#FF5F57", "#FEBC2E", "#28C840"].map((c, i) => <div key={i} style={{ width: 10, height: 10, borderRadius: "50%", background: c }} />)}
              <span style={{ marginLeft: 10, fontSize: 10, color: "rgba(0,255,209,0.3)", letterSpacing: "0.15em" }}>CODON_CONTACT v1.0</span>
            </div>
            {submitted ? (
              <div style={{ textAlign: "center", padding: "40px 0" }}>
                <div style={{ fontSize: 44, marginBottom: 18 }}>🎉</div>
                <h3 style={{ fontFamily: "'Syne',sans-serif", fontSize: 20, color: "#00FFD1", marginBottom: 12 }}>Sequence Initialized.</h3>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {[["name", "YOUR NAME", "text", "Alex Johnson"], ["email", "EMAIL", "email", "alex@email.com"]].map(([id, label, type, ph]) => (
                  <div key={id}>
                    <label style={{ display: "block", fontSize: 10, letterSpacing: "0.25em", color: "rgba(0,255,209,0.5)", marginBottom: 6 }}>{label}</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 10, letterSpacing: "0.25em", color: "rgba(0,255,209,0.5)", marginBottom: 6 }}>I NEED A...</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 10, letterSpacing: "0.25em", color: "rgba(0,255,209,0.5)", marginBottom: 6 }}>TELL US ABOUT YOUR PROJECT</label>
...
function Footer() {
  return (
    <footer style={{ borderTop: "1px solid rgba(0,255,209,0.15)", padding: "36px 5%" }}>
      <div className="footer-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          {[["IG", "Instagram"], ["𝕏", "Twitter"], ["in", "LinkedIn"], ["▶", "YouTube"]].map(([n, label]) => (
            <a key={n} href="#" title={label} style={{ width: 34, height: 34, borderRadius: "50%", border: "1px solid rgba(0,255,209,0.12)", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(232,232,240,0.4)", fontSize: 13, transition: "all 0.2s" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "#00FFD1"; e.currentTarget.style.color = "#00FFD1"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(0,255,209,0.12)"; e.currentTarget.style.color = "rgba(232,232,240,0.4)"; }}
            >{n}</a>
          ))}
        </div>
      </div>
